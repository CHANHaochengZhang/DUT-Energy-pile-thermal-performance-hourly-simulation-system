...
if st.session_state['page'] == 0:
    ...
    # ===== Sidebar：功能介绍 =====
    st.sidebar.title("📚 平台功能介绍")

    st.sidebar.markdown("点击以下卡片，展开对应功能的详细介绍👇")

    with st.sidebar.expander("📌 功能一：全年热-力性能计算", expanded=False):
        st.markdown("""
        - 上传建筑全年逐时冷热负荷
        - 输入能量桩设计参数：支持螺旋形、U形、W形能量桩
        - 加载物理核函数模块，内嵌：
            - 地温二谐波模型
            - 格林函数线性热源核响应函数
            - 格林函数圆形热源核响应函数
        - 加载或支持用户自定义XGBoost热泵模型
        - 全年逐时热-力指标计算：
            - 不同深度桩壁温度
            - 桩群出口水温
            - 桩体力学形变
    ...
    with st.sidebar.expander("🔧 功能二：能量桩设计参数优化", expanded=False):
        st.markdown("""
        - 自定义优化空间
        - 自定义GBR训练样本采样
        - 自定义GBR模型结构：
    ...
    with st.sidebar.expander("🚀 功能三：运行策略优化展望", expanded=False):
        st.markdown("""
        - 基于稀疏动力学的太阳能辅助–能量桩–热泵–建筑系统预测控制（MPC）框架构建，研究展望
        - SINDy模型训练
        - 模型预测controller设计
        """)
    ...
    st.sidebar.markdown(
        ...
        <div class="team-card">
            <h4>🏫 开发团队 & 单位</h4>
            <ul>
                <li>大连理工大学<br>建筑环境与设备工程研究所</li>
                <li>丹麦技术大学<br>Solar Group</li>
            </ul>
            <h4>👩‍💻 开发成员</h4>
            <ul>
                <li>徐懿</li>
            ...
            <div class="team-contact">📧 联系方式：</div>
            <div>xuyi981007@163.com</div>
            <div class="team-copyright">&copy; 2025 能量桩热-力性能模拟系统 · 版权所有</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # 技术路线图
    st.image("tech_road.jpg", caption="系统开发技术路线图", use_container_width=True)
    ...
    with col1:
        with st.container():
            if st.button('🟢 功能一', key='func1', help='包含负荷上传、参数设定、模拟计算和结果可视化', use_container_width=True):
                st.session_state['page'] = 1
                st.rerun()
            st.markdown(
                '<div style="background:#f0f0f0;padding:8px;border-radius:8px;text-align:center;margin-top:4px;width:100%;">全年热-力性能计算</div>',
                unsafe_allow_html=True
            )

    with col2:
        with st.container():
            if st.button('🟡 功能二', key='func2', help='使用代理模型与遗传算法优化能量桩参数', use_container_width=True):
                st.session_state['page'] = 12
                st.rerun()
            st.markdown(
                '<div style="background:#f0f0f0;padding:8px;border-radius:8px;text-align:center;margin-top:4px;width:100%;">设计参数优化</div>',
                unsafe_allow_html=True
            )

    with col3:
        with st.container():
            if st.button('🚀 功能三', key='func3', use_container_width=True):
                st.session_state['page'] = 13
                st.rerun()
            st.markdown(
                '<div style="background:#f0f0f0;padding:8px;border-radius:8px;text-align:center;margin-top:4px;width:100%;">运行策略优化展望</div>',
                unsafe_allow_html=True
            )
    ...
    div.stButton > button {
        ...
        justify-content: flex-start !important; /* 让文字往左靠 */
    }
    </style>
    """,
    unsafe_allow_html=True,
)
    st.markdown("本页功能：在本页面进行建筑冷热负荷全年逐时数据上传、加载与预览")
...
#         page_nav()
    ...
    **📖 本页内容说明：**
    ...
    1️⃣ 请按提示在左侧 **Sidebar** 输入地理信息及地温二谐波模型的相关参数；  
    2️⃣ 在右侧 **主页面** 填写能量桩的设计参数；  
    3️⃣ 计算支持在桩壁垂直方向设置多个 **均匀计算节点 n** 作为温度输出点，⚠️ **需确保输入的 n 包含桩壁中点；**  
    4️⃣ 完成所有参数输入后，请将 **右侧参数栏** 滑至底部，点击 **“提交参数”** 按钮以开始计算🚀。 
    """
)
       ...
       # ===== 左边栏：地理参数设定 =====
        with st.sidebar:
            st.header("地理参数设定")
            st.markdown(
                "<span style='color:black; font-size:18px; font-weight:bold;'>⚠️ 请选择参数并点击底部按钮确认，否则不会生效</span>",
                unsafe_allow_html=True
            )
...
- 根据设计参数计算能效比 s 及相关热阻参数
    ...
    st.info(f"**能效比 s**: {s:.4f}  （管式类型: {pipe_type}）")
    st.success(f"**单根管道流量**: {M:.2f} kg/h")
    st.success(f"**埋管总长度 L_total**: {L_total:.2f} m")
    st.success(f"**体积流量 qv**: {qv:.2f} m³/h")
    st.success(f"**雷诺数 Re**: {Re:.2f}")

    zz = params['zz']
    st.success("垂直分层zz深度 (m): " + ", ".join([f"{x:.2f}" for x in zz]))
    ...
    if st.button('返回首页', key='back_home_X'):
        st.session_state['page'] = 0
        st.rerun()
    st.header("4. 土壤温度场年周期变化建模及地温二谐波模型")

    # ===== Sidebar：功能介绍 =====
    st.sidebar.title("📘 地温二谐波模型与卷积计算说明")

    st.sidebar.markdown("""
### 🌡️ 地温二谐波模型（dynamic_model）
- **来源**：基于半无限大介质的周期性传热理论，仝仓等 [1] 对大连地区实测数据进行拟合，得到二谐波形式的地温模型。  
- **作用**：用于描述地表温度 $T_s(t)$ 随时间的周期性变化，并衰减传播至深度 $z$ 处：  
  $T(t,z) = T_{s,avg} - \\text{谐波项1} - \\text{谐波项2}$  
  该函数在代码中实现为 `dynamic_model(t, z)`，即地表动态边界的数学表达，经`dynamic_model(t, z)`计算所得结果记为 $T(t,z)$。

---

### 🔹 初始温度分布（T_initial）
- **定义**：在未考虑外部热源作用下，仅由地温周期性边界（`dynamic_model`）传播至土壤内部所形成的**基线温场**。  
- **实现**：通过对 `dynamic_model` 在虚拟深度方向的积分卷积，结合傅里叶热传导核函数，得到每个深度点在全年范围内的基线温度：  
  $T_{initial}(t,z) = \\int_0^\\infty f(\\xi)\\, T_s(0,\\xi)\\, d\\xi$  
  （物理含义：初始条件的传热响应）

---

### ⚡ 动态边界响应（T_gs）
- **定义**：不考虑初始条件与内部热源，仅由地表温度周期扰动（`dynamic_model`）在时间 $t$ 传播至深度 $z$ 的**瞬时响应**。  
- **实现**：通过对地表边界 $T_s(\\tau)$ 与传热格林函数的**时间卷积**：  
  $T_{gs}(t,z) = \\int_0^t T_s(\\tau,0)\\, g(t-\\tau,z)\\, d\\tau$  
  其中 $g(t-\\tau,z)$ 表示热扩散的瞬时脉冲响应。

---

### 🔗 关系总结
- `dynamic_model(t,z)`：提供地表温度边界条件（输入信号$T_s(t,z)$）。  
- `T_initial(t,z)`：在无扰动热源时，初始条件作用下的温度分布。  
- `T_gs(t,z)`：在无初始条件时，边界条件作用下的温度响应。  
- **二者叠加**，可构成完整的土壤温度场时空分布基线。
                        
---

### 🖥️ 向量化卷积算法
- **问题**：$T_{initial}$ 与 $T_{gs}$ 的计算均包含卷积积分，直接数值积分效率低。  
- **改进**：通过 NumPy 向量化与 `@jit(nopython=True)` 加速：
    - 将积分域离散化为向量；
    - 预计算高频调用的指数与权重；
    - 使用 `np.trapezoid` 实现快速积分。
- **效果**：显著减少循环计算，提升全年逐时温度场模拟速度。

---

📖 参考文献  
[1] Tong C, Li X, Duanmu L, et al. *Prediction of the temperature profiles for shallow ground in cold region and cold winter hot summer region of China*. **Energy and Buildings**, 2021, 242: 110946.
""")

    st.markdown("### 📌 本页内容概览")

    with st.expander("🌡️ 动态边界模型 `dynamic_model`"):
        st.markdown(
            "- 根据地温二谐波模型，定义动态边界模型 `dynamic_model`"
        )

    with st.expander("🔹 初始温度分布计算`T_initial`"):
        st.markdown(
            "- 根据地表温度的二谐波传播模型和一维热传导理论，计算每一深度点在时间范围内的初始温度分布 (`T_initial`)，即在未受热负荷扰动前的温场基线。"
        )

    with st.expander("⚡ 地表动态温度响应`T_gs`"):
        st.markdown(
    ...
    st.markdown(
    '<span style="color:black; font-weight:bold;">⚠️ 请等待计算完成并绘制出图像后再点击"下一页"按钮</span>', 
    unsafe_allow_html=True
)
    params = st.session_state['params']
    # 取参数
    ...
    if st.button('返回首页', key='back_home_X'):
        st.session_state['page'] = 0
        st.rerun()
    st.header("5. 格林函数核响应函数预计算")

    # ===== Sidebar：功能介绍 =====
    st.sidebar.title("📘 几何核函数与积分模型设计说明")

    st.sidebar.markdown(
        """
    **核心目标**：高效计算不同埋管几何下的格林函数核响应，并构造热响应矩阵，用于后续温度场模拟。

    ### 📐 几何核函数定义

    - **circle() 圆形热源模型**  
    - 基于互补误差函数 Erfc 的瞬态点热源解  
    - 返回环向热源对土体某点瞬时温升的微元贡献
    ...
    - **circle 积分**  
    $$
    G_{circle}(t) = \\int_0^{2\\pi} circle(w,t,...)\\,dw
    $$
    得到整环热源在时间 $t$ 的温升响应。

    - **line 积分**  
    $$
    G_{line}(t) = \\int_{z_{min}}^{z_{max}} line(h,t,...)\\,dh
    $$
    得到整条线热源在时间 $t$ 的温升响应。
    ...
    - **compute_kernels_all_points()**  
    - 根据 `pipe_type` 自动选择 circle / line 积分方式  
    - 在 $(x,y,z,t)$ 网格上并行计算核响应  
    - 整合结果为**热响应矩阵**，缓存供后续仿真调用  
    - 基于 `joblib.Parallel` 并行化积分计算  
    - 结合进度条实时反馈，显著缩短逐点逐时的核函数积分时间

    ---

    📖 注：  
    不同 `pipe_type` 与积分函数调用关系：  
    - 螺旋管 → circle + line  
    - U型管 → line1 + line2  
    - W型管 → line1 + line2 + line3 + line4
    """
    )

    st.markdown("### 📌 本页内容概览")

    with st.expander("🔹 核心函数说明及其使用说明"):
        st.markdown(
            """
    1. **几何核函数定义**  
    - `circle(w, t, ...)`：圆形热源瞬态响应（螺旋管环向近似）。
    - `line(h, t, ...)`：直线热源瞬态响应（轴向连续近似）。
    2. **几何积分实现**  
    - `compute_circle_integrals()`：对 `circle()` 在 $w \\in [0,2\\pi]$ 数值积分，得到整环响应。
    - `compute_line_integrals()`：对 `line()` 在 $z \\in [z_{min},z_{max}]$ 数值积分，得到整线响应。
    3. **整体核函数预处理**  
    - `compute_kernels_all_points()`：调用上述积分模块，整合所有深度点 $z$ 与时间步 $t$ 的响应。
            """
        )

    with st.expander("🔹 管式类型与调用关系"):
        st.markdown(
            """
    - **螺旋管**（pipe_type="螺旋管"）  
    调用 `compute_circle_integrals()` + `compute_line_integrals()` → 获取环向与轴向耦合响应。  
    - **U型管**（pipe_type="U型管"）  
    调用 `compute_line1_integrals()` + `compute_line2_integrals()` → 对称双管响应。  
    - **W型管**（pipe_type="W型管"）  
    调用 `line1/line2/line3/line4` 积分核函数 → 多分支响应叠加。
            """
        )
    ...
    with st.expander("✅ 最终输出"):
        st.markdown(
            """
    - 全部深度点 × 空间响应点 × 时间步 的核响应矩阵  
    - 缓存至 `st.session_state`，供后续温度场叠加模拟直接调用
            """
        )


    st.markdown('<span style="color:black; font-weight:bold;">- ⚠️ 请等待所有核响应函数计算完成后再点击"下一页"按钮</span>', unsafe_allow_html=True)
    ...
    if st.button('返回首页', key='back_home_X'):
        st.session_state['page'] = 0
        st.rerun()
    st.header("6. 热泵模型训练或加载")

    st.sidebar.title("📘 热泵性能XGBoost回归建模说明")

    st.sidebar.markdown(
    """

    **研究思路**  
    - 本研究引入机器学习方法，对热泵机组运行性能进行建模与预测。  
    - 目标：根据运行条件，快速拟合并预测机组能效指标 **COP**。  
    ...
    ### 🔹 数据处理与训练  
    - 收集系统在不同工况下的输入-输出实验数据；  
    - 预处理：缺失值清洗与补全；  
    - 数据划分：使用 `train_test_split` 按 1:1 分为训练集与测试集；  
    - 使用 **XGBoost Regressor** 进行训练，并设置随机种子保证可重复性。  

    ---

    ### 📊 模型精度
    <span style="color:red; font-weight:bold;">已有模型精度如下图所示：</span>  
    """, unsafe_allow_html=True
)

    st.sidebar.image("HP-cop.jpg", caption="预加载热泵模型COP预测效果展示")

    st.markdown("### 📌 本页内容概览")

    with st.expander("🛠️ 自定义热泵模型"):
        st.markdown(
            "- 自定义热泵模型，根据用户提供的数据进行热泵模型训练。"
        )
...
**💡 为什么要初始化？**
- ✅ 将所有用于全年逐时模拟的变量一次性按 `ttmax` 长度（或相应形状）创建并存入 `st.session_state`，保证后续计算有一致、可复现的“起点”。
- ✅ 避免后续模块因缺少变量或形状不一致而报错；便于调试与结果复现。
- ✅ 初始化是轻量级的准备步骤——只有在你点击 **“执行系统初始化”** 按钮后才会真正写入 `session_state`（未点击前不会改变全局状态）。

**🎯 初始化的意义**
- 创建并保存（示例变量名）：
  - `Q1`, `q`, `T_line`, `T_circle`, `T_point`：长度为 `ttmax` 的零数组（逐时量阵列）。
  - `all_integral_circle_result`, `all_integral_line_result`：用于保存核函数积分结果，形状为 `(ttmax, 6)`（示例）。
  - `T_in`, `T_out`：`T_out` 长度为 `ttmax+1`，用于保存出口温度的时序（含初始值）。
  - `COP`, `PLR`：热泵性能与部分负荷率，长度为 `ttmax`。
  - 力学相关初始量：`z_top`, `sigma_max`（长度为 `ttmax`）。
- 初始出口温度 `T_out[0]` 的赋值逻辑：
  - 若 `T_initial` 是二维数组（深度 × 时间），程序会**自动取最接近桩中点深度 H/2 的那一层**并使用其在时刻 0 的值作为 `T_out[0]`；
  - 否则直接取 `T_initial[0]` 作为 `T_out[0]`。
  这样能保证出口水温初始化与桩体温场的物理一致性。

**🔘 操作步骤**
- 点击页面中的 **“⚡ 执行系统初始化”** 按钮触发真正的写入与初始化流程；
...
""")
    ...
    # 增加一个按钮来触发初始化
    if st.button("⚡ 执行系统初始化", key="init_system"):
        try:
            ttmax = st.session_state['params']['ttmax']
            T_initial = st.session_state['T_initial']
            zz = st.session_state['params']['zz']
    ...
    st.header("8. 全年模拟")
    # ================= Sidebar 部分 =================
    st.sidebar.title("📘 全年逐时动态仿真建模与验证说明")
    st.sidebar.markdown("""
    ### 📖 动态仿真建模说明  
    - 本页基于 **热泵模型** 与 **换热解析模型**，开展全年逐时动态仿真。  
    - 其核心思想是将 **建筑负荷 → 热泵性能 → 地下换热器温度响应 → 力学响应** 串联起来，逐步迭代推进。 
    """)

    with st.sidebar.expander("👇 点击展开详细流程说明", expanded=False):
        st.markdown("""
                            
        1. **输入参数**  
        包括土壤热物性（比热、导热率、扩散率）、能量桩结构与布置（数量、直径、埋深、螺旋管半径与螺距等）、回填材料参数、热泵运行条件（流速、比热容）、时间步长与模拟周期，以及逐时建筑负荷曲线。  

        2. **初始条件计算**  
        根据地温二谐波模型，获得土壤温度场与桩体初始温度分布，作为仿真的起点。  

        3. **逐时动态迭代**  
        - 判断当前负荷工况（制冷/制热）；  
        - 调用热泵模型计算 COP 与机组负荷分配；  
        - 结合换热解析模型计算地下换热器的传热量与桩壁温度分布；  
        - 更新桩群出口水温 **T_out** 与下一时刻的入口条件。  

        4. **力学计算耦合**  
        根据不同深度的桩壁温度差异，计算桩体的热膨胀、顶端位移及热应力。  

        5. **时间推进与循环**  
        时间步逐一推进，直到覆盖整个模拟周期（全年8760小时）。  

        6. **输出结果**  
        - 热力学：全年桩群出口水温、桩壁温度场分布；  
        - 力学：桩顶位移、最大热应力。  

        该流程实现了建筑负荷、热泵性能与能量桩换热/力学行为的耦合模拟，可用于系统性能评估与设计验证。
    ...
    st.sidebar.markdown("""
    ### ⚙️ 换热解析模型支持下的力学性能验证  
    - 使用 C30 混凝土参数：线膨胀系数与弹性模量；  
    - 根据不同深度桩壁温度，计算桩顶位移与最大热应力；  
    - 所得结果与文献保持同一数量级，验证了模型合理性。  

    📚 参考文献：  
    孔纲强, 王成龙, 刘汉龙, 等. 多次温度循环对能量桩桩顶位移影响分析[J].  
    岩土力学, 2017, 38(04): 958-964. DOI:10.16285/j.rsm.2017.04.005.

    ---

    ### 🔍 系统热力计算输出结果验证说明  
    <span style="color:red; font-weight:bold;">计算搭载换热解析模型桩群出口水温与 TRNSYS 输出对比</span>    
    """, unsafe_allow_html=True)

    st.sidebar.image("Tout-verification.jpg", caption="计算搭载换热解析模型桩群出口水温与TRNSYS输出对比")
    ...
    with st.expander("📝 调用已有输入参数"):
        st.markdown("""
        - 自动读取前面页面已经设置的 **设计参数** 与 **热泵模型**；
        - 这些输入将作为全年逐时模拟的 **基础条件**；
        - 无需重复输入，直接点击下方按钮即可开始模拟。
        """)

    with st.expander("📊 输出结果说明", expanded=True):
        st.markdown("""
        模拟完成后，将生成以下关键结果：
        - 🌊 **桩群出口水温**（逐时变化曲线）；  
        - 🌡️ **桩壁温度分布**（不同深度节点）；  
    ...
    st.markdown("""
    ### ▶️ 启动模拟计算
    请点击下方按钮，运行全年逐时模拟并获取结果。
    """)
    
     # 计算按钮
    if st.button("▶️ 开始全年模拟计算", key="start_annual_sim"):
    # 检查是否已有缓存结果
        if 'annual_simulation_done' in st.session_state and st.session_state['annual_simulation_done']:
            st.info("检测到已缓存的全年模拟结果，跳过计算。如需重新计算请刷新或重置。")
        else:
            try:
                params = st.session_state['params']
    ...
    st.markdown('<span style="color:black; font-weight:bold;">- ⚠️ 请等待右边"下一页"按钮由灰色变为红色后再点击"下一页"按钮</span>', unsafe_allow_html=True)
    page_nav()
